...
export interface CompanyAssumedCancellationCreateBody {
  inbound_order_id: number;
  reason?: string | null;
...
export interface PurchaseReturnOrderOut {
  ...
  customer_refund_amount: number | string;
  company_loss_amount: number | string | null;
  reason: string | null;
  submitted_at: string;
...
export interface CustomerRefundOut {
  id: number;
  no: string;
  purchase_return_order_id: number;
  sales_order_id: number;
...
export interface CompanyLossEntryOut {
  id: number;
  no: string;
  purchase_return_order_id: number;
  payable_id: number;
...
export interface PurchaseReturnDetail {
  ...
  customer_refund: CustomerRefundOut | null;
  company_loss_entry: CompanyLossEntryOut | null;
}
...
export const purchaseReturnApi = {
  ...
  createCompanyAssumedCancellation: (body: CompanyAssumedCancellationCreateBody) =>
    api.post<PurchaseReturnDetail>("/api/v1/purchase-returns/company-assumed-cancellations", body),
  approve: (id: number) =>
  ...
  confirmCompanyAssumedCancellation: (
    id: number,
    body: { cancellation_reference?: string | null; cancellation_note?: string | null },
  ) =>
    api.post<PurchaseReturnDetail>(
      `/api/v1/purchase-returns/${id}/confirm-company-assumed-cancellation`,
      body,
    ),
};
