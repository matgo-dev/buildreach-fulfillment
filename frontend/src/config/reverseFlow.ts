...
export const REVERSE_FLOW_PRINCIPLES = [
  ...
  "库存当前按销售单维度落库;采购退货会扣减原销售单维度库存,不会直接进入自由库存。",
  "入库后涉及钱或货的回退,用采购退货、供应商贷项单、收付款等真实单据承载,不再用空泛申请单承载。",
];
        ...
        children: [
          {
            ...
            result: "当前线上入口会被拦截;供应商接受时走采购退货单;退货出库后生成供应商贷项单,财务过账后冲减未结应付。",
            severity: "goods",
          },
          {
        ...
        children: [
          {
            ...
            result: "指定核销记录被反核销;收款单未分配金额和未结应收恢复对应金额。已出库后的客户退货不靠反核销回退原单。",
            severity: "money",
          },
          {
            id: "unwind-shipment",
...
export const REVERSE_FLOW_BOUNDARIES = [
  "这张图覆盖出库确认前的取消、撤销、作废、采购退货、供应商贷项单和反核销等基础逆向动作。",
  "出库确认后原流程不可逆;客户退货、退款、贷项、跨期冲销或库存恢复需走独立售后流程。",
  "出库后异常由售后型真实单据承载;原销售、采购、入库、出库单不再倒回。",
