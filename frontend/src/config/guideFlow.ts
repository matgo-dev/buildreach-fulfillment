...
export const GUIDE_NODES: GuideNode[] = [
  ...
  },
  {
    ...
    band: "SOURCING",
    inEdgeLabel: "供应商发货",
    tooltip: "供应商一发货就登记在途,货真的到了再确认入库。",
    what: "分两步走:供应商一发货,仓库先登记一张「在途」的入库单;等货真的到仓库、点过数了,再「确认入库」。",
    fromTo: "上一步:已确认的采购单(只有确认过的采购单才能收货)。下一步:确认入库后货就进了库存。在左侧菜单「仓储物流」→「入库单」里操作。",
    effect: "确认入库的那一刻,这批货才算真的到了,库存里才看得见。同一时刻系统自动记下「我们欠供应商这笔钱」。",
    narration: "一个月后瓷砖厂发货,仓库先登记在途;货到仓库点清 200 套,确认入库。",
  },
  {
    id: "inventory",
    iconKey: "inventory",
    ...
    band: "SOURCING",
    inEdgeLabel: "仓库货变多了",
    tooltip: "按销售单看这批货到了多少、发了多少、还能发多少。",
    what: "这里不是传统的仓库流水账,而是按销售单看每样货的执行情况:订了多少、到了多少、已经发了多少、还能发多少。我们的货从采购起就归属某一张销售单,所以不存在「谁都能拿」的公共库存。",
    fromTo: "上一步:确认入库。下一步:要发货时开出库单。在左侧菜单「仓储物流」→「库存」里查看。",
    effect: "库存数字不是单独记的一本账,而是拿「入库减出库」当场算出来的 —— 所以它永远和单据对得上,不会出现账实不符。",
    narration: "库存页上这张销售单的瓷砖显示:到货 200,已发 0,可发 200。",
  },
  ...
  },
  {
    ...
    layer: "MONEY",
    anchorId: "inbound",
    inEdgeLabel: "货一收到,开始欠供应商钱",
    tooltip: "确认入库时自动产生,不用手工建。",
    what: "货收进仓库的那一刻,系统自动记一笔「我们欠供应商多少钱」。和应收一样,是自动产生的。",
    fromTo: "上一步:确认入库。下一步:付款后登记付款单来冲抵。在左侧菜单「财务」→「应付款」里查看。",
    effect: "应付款的状态(未付 / 部分付 / 已付清)按已经付了多少自动算。",
    narration: "当初收货的时候,系统就已经记下了我们欠瓷砖厂的货款。",
  },
  {
    id: "payment",
    iconKey: "payment",
