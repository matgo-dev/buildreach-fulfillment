...
export const DRAFT_FLOW_NODES: DraftFlowNode[] = [
  ...
  },
  {
    ...
    band: "ORDERING",
    inEdgeLabel: "供应商开始动货",
    note: "草案里,入库单创建不再只是登记在途,而是表示供应商已发货或我们已去拉货。",
    sideEffects: ["产生客户应收", "产生供应商应付", "进入不可简单回退段"],
    boundary: true,
  },
  {
    id: "receiveInbound",
    ...
    category: "GOODS",
    band: "FULFILLMENT",
    note: "货到货代仓后确认入库,库存随之更新;这个节点不再新产生应收/应付。",
  ...
  },
  {
    ...
    band: "FULFILLMENT",
    inEdgeLabel: "选择货和柜",
    note: "按超总最新口径,出库单一旦形成,原正向流程不可逆;后续退货退款必须新建逆向流程。",
  ...
  },
  {
    ...
    band: "FULFILLMENT",
    inEdgeLabel: "货装进柜",
    note: "确认出库只负责扣减库存和确认装柜事实,不再新产生客户应收。",
    sideEffects: ["扣减库存"],
  },
  {
    id: "shipment",
...
export const DRAFT_MONEY_BRANCHES: DraftMoneyBranch[] = [
  {
    ...
    role: "FINANCE",
    inEdgeLabel: "同时生成",
    note: "应收从原来的确认出库前移到入库单创建/发货拉货确认。客户后续付款或已有预收再核销到这笔应收。",
    settlement: "后续用收款单核销;先收款则形成预收再核销",
  },
  {
    id: "payable",
    anchorId: "dispatchConfirm",
