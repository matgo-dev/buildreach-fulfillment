...
- **selectFilter（列头单选筛选，`components/common/SelectFilter`）**：次要枚举列（币种/采购进度/SKU 状态等）的**唯一**筛选载体——**下拉尖头**图标（非漏斗）+「全部 + 各枚举值」清单，**点选即生效即关闭**（无单选钮、无「确定」）。展开进列定义 `...selectFilter(options)`；服务端过滤再配 `filteredValue`（受控），客户端过滤配 `onFilter`。**禁用 AntD 内置 `filters` 面板**（理由见 §7 点击成本）。
...
- **列表排序**：时间列（及金额等数值列）默认可排序，表头 ↕/↓。红线金额列对无权角色整列不渲染（见 §9），排序问题随之消失。
- **列表工具条统一次序**（所有列表页一致，报价管理为范式）：**[单据状态 Segmented（全部+各态，≤5 平铺）] → [搜索框] → [参照维度下拉（供应商等，可选）] → [页面特有开关]**。状态永远最左、搜索紧随其后。
- **次要枚举列不占工具条**：币种/采购进度等**次要枚举维度走列头下拉筛选**（`selectFilter`，见 §5；触发服务端过滤），工具条只留主轴（状态）+搜索+参照维度。基数 >5 的参照维度（品类/供应商）→ 下拉。
- **点击成本是硬约束**：任何单选控件**点选即生效**，绝不「勾选 → 再点确定」多要一次点击；列表筛选一律「开→选」两步到位。AntD 内置列头 `filters` 面板（单选钮 + 重置/确定 = 三步）因此在本平台**禁用**，统一走 `selectFilter`。这条不限于筛选：单选场景出现两段式确认即为违规（多值提交、危险操作除外，后者的确认讲的是业务后果）。
...
**渲染层怎么藏：整列 / 整项不渲染，不是显示「—」。** 后端脱敏是安全底线，渲染层这一道纯粹是体验：
- **不留占位**。一整列「—」不传递任何信息，却占着表格宽度，还会带出「点了没反应」的死排序。行业主流也是移除而非遮罩（Salesforce 字段级安全把无读权限字段从布局 / 列表 / 报表全部移除；Odoo 字段挂 `groups=` 直接不渲染；SAP Fiori 显示态不展示空字段）。
- **落点**：`useAuthStore((s) => s.hasPermission(Permissions.XXX))` 判可见性 → 列数组条件展开、`Descriptions.Item` 条件渲染。门就是后端那一个权限点（售价=`receivable:read`，成本=`purchase:read_cost`），前后端同一个门，不另发明。
- **仍保留 `formatCost` / `formatPrice`**（`null → 「—」`）作为兜底：万一前端权限镜像与后端脱敏漂移，渲染出来是「—」而不是 `0.00` 这种看着像真值的假金额。
- 与「单据链接降级」的区别：单号**不是**红线，所以号照显、只降级掉点击；红线值本身不可见，所以整个藏掉。
