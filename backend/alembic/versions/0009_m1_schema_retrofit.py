...
- customers.preferred_language → 重命名 quote_language + 收窄 String(10)
  (原 BCP47 自由串 String(35) 唯一消费者是 resolve_quote_language 映射;报价语言业务上
   只有 zh/en/sw 三选一,砍掉宽存窄用的中间层。值域唯一源头是 core.languages 常量,
   校验走应用层,DB 不内联枚举副本——避免与常量并列两份)
...
def upgrade() -> None:
    ...
    # 值域校验走应用层(单一源头 core.languages 常量),DB 不内联枚举副本,仅重命名 + 收窄。
