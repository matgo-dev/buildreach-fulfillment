...
核销引擎候选查询形状 = WHERE 对手方 + 币种 + voided_at IS NULL + balance>0
...
AND balance>0。断言:无 Seq Scan(不全表扫)且无 Sort(排序走进索引)——两者共同锁定该索引被用。
...
async def test_receivable_aging_query_hits_partial_index(db_session):
    plan = await _plan(db_session,
        ...
        "WHERE customer_id = 1 AND currency = 'USD' AND voided_at IS NULL AND balance > 0 "
...
async def test_payable_aging_query_hits_partial_index(db_session):
    plan = await _plan(db_session,
        ...
        "WHERE supplier_id = 1 AND currency = 'USD' AND voided_at IS NULL AND balance > 0 "
        "ORDER BY due_at ASC NULLS LAST, created_at ASC, id ASC")
