...
@pytest.mark.asyncio
async def test_create_spu_then_sku_builds_search_text(client, superadmin_headers, db_session):
    await _seed_category(db_session)

    r_spu = await client.post("/api/v1/spus", headers=superadmin_headers,
                              json={"category_code": "10", "name_i18n": {"zh": "球阀"}})
    assert r_spu.status_code == 200, r_spu.text
    spu_id = r_spu.json()["data"]["id"]

    r_sku = await client.post("/api/v1/skus", headers=superadmin_headers, json={
...
@pytest.mark.asyncio
async def test_update_sku_recomputes_search_text(client, superadmin_headers, db_session):
    await _seed_category(db_session)
    ...
    r = await client.put(f"/api/v1/skus/{sku_id}", headers=superadmin_headers, json={
...
@pytest.mark.asyncio
async def test_create_sku_rejects_duplicate_spec_key(client, superadmin_headers, db_session):
    await _seed_category(db_session)
    spu_id = (await client.post("/api/v1/spus", headers=superadmin_headers,
              json={"category_code": "10", "name_i18n": {"zh": "球阀"}})).json()["data"]["id"]
