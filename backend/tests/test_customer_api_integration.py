...
@pytest.mark.asyncio
async def test_create_and_list_customer(client, sales_headers):
    ...
    r2 = await client.get("/api/v1/customers", headers=sales_headers)
    data2 = r2.json()["data"]
    assert any(c["id"] == data["id"] for c in data2["items"])


@pytest.mark.asyncio
async def test_create_customer_rejects_empty_name(client, sales_headers):
...
@pytest.mark.asyncio
async def test_create_customer_rejects_bad_quote_language(client, sales_headers):
...
@pytest.mark.asyncio
async def test_list_filter_by_status_and_keyword(client, sales_headers):
    """列表:状态 tab + 关键词(code/name/联系人)筛选 + 分页信封。"""
    await client.post("/api/v1/customers", headers=sales_headers,
                      json={"name": "阿尔法建材", "contact_name": "李工"})
    b = (await client.post("/api/v1/customers", headers=sales_headers,
                           json={"name": "贝塔工程"})).json()["data"]
    await client.post(f"/api/v1/customers/{b['id']}/deactivate", headers=sales_headers)
...
@pytest.mark.asyncio
async def test_get_and_update_customer(client, sales_headers):
    c = (await client.post("/api/v1/customers", headers=sales_headers,
                           json={"name": "旧名", "quote_language": "zh"})).json()["data"]
    g = (await client.get(f"/api/v1/customers/{c['id']}", headers=sales_headers)).json()["data"]
    assert g["name"] == "旧名"
...
@pytest.mark.asyncio
async def test_update_rejects_bad_quote_language(client, sales_headers):
    c = (await client.post("/api/v1/customers", headers=sales_headers,
                           json={"name": "语言校验"})).json()["data"]
    r = await client.put(f"/api/v1/customers/{c['id']}", headers=sales_headers,
                         json={"name": "语言校验", "quote_language": "sw-TZ"})
    assert r.status_code == 422


@pytest.mark.asyncio
async def test_toggle_status_idempotent(client, sales_headers):
    """启停切换幂等:重复 deactivate 稳定 INACTIVE;再 activate 回 ACTIVE。"""
    c = (await client.post("/api/v1/customers", headers=sales_headers,
                           json={"name": "切换客户"})).json()["data"]
    d1 = await client.post(f"/api/v1/customers/{c['id']}/deactivate", headers=sales_headers)
    d2 = await client.post(f"/api/v1/customers/{c['id']}/deactivate", headers=sales_headers)
    assert d1.status_code == d2.status_code == 200
    assert d2.json()["data"]["status"] == "INACTIVE"
    a = await client.post(f"/api/v1/customers/{c['id']}/activate", headers=sales_headers)
    assert a.json()["data"]["status"] == "ACTIVE"


@pytest.mark.asyncio
async def test_customer_not_found(client, sales_headers):
    assert (await client.get("/api/v1/customers/999999",
                             headers=sales_headers)).status_code == 404
...
@pytest.mark.asyncio
async def test_admin_cannot_access_customers(client, superadmin_headers):
    """ADMIN 已摘除 customer:*(Q25 纯系统域):读写均 403。需管客户给账号叠加 SALES 角色。"""
    assert (await client.get("/api/v1/customers",
                             headers=superadmin_headers)).status_code == 403
    assert (await client.post("/api/v1/customers", headers=superadmin_headers,
                              json={"name": "x"})).status_code == 403
